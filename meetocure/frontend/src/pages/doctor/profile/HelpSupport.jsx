  ...
  const faqs = [
    {
      ...
      answer:
        "To book an appointment, go to the dashboard, select a category or search for a doctor, choose your preferred doctor, select an available time slot, and confirm your booking. You'll receive a confirmation via SMS and email.",
    },
    {
      question: "Can I cancel or reschedule my appointment?",
      answer:
        "Yes, you can cancel or reschedule appointments up to 24 hours before the scheduled time. Go to 'My Appointments' in your profile, select the appointment, and choose to cancel or reschedule.",
    },
    {
      question: "How do I update my profile information?",
      answer:
        "Navigate to your profile page and click on 'Edit Profile'. You can update your personal information, contact details, and profile picture. Changes are saved automatically.",
    },
    {
      question: "What payment methods are accepted?",
      answer:
        "We accept various payment methods including UPI, credit/debit cards, net banking, and digital wallets like Paytm, PhonePe, and Google Pay. All payments are secure and encrypted.",
    },
    {
      question: "How do I contact customer support?",
      answer:
        "You can reach our customer support team through multiple channels: in-app chat, phone support, email, or WhatsApp. Our team is available 24/7 to assist you with any queries.",
    },
  ];
      ...
      <div className="max-w-4xl mx-auto px-6 mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {supportChannels.map((channel) => (
            <div
              key={channel.title}
              className={`rounded-xl shadow-sm p-6 flex flex-col items-center cursor-pointer transition-colors ${channel.color} ${channel.hoverColor}`}
              onClick={() => handleSupportAction(channel)}
              tabIndex={0}
              role="button"
              onKeyPress={(e) => {
                if (e.key === "Enter" || e.key === " ") handleSupportAction(channel);
              }}
            >
              {channel.icon}
              ...
              <button
                type="button"
                className="bg-white text-[#0A4D68] px-3 py-1 rounded-lg font-semibold mt-2"
              >
                {channel.action}
              </button>
            </div>
